...
st.set_page_config(
    page_title="따릉이 신규 정류소 배치를 통한 기대효과 대시보드 - 남현동",
    layout="wide",
)  # Streamlit 페이지 설정(가로 폭 넓게)

st.markdown(
    """
    <style>
      /* 화면 풀사이즈 채우기: 폭 제한 제거 + 패딩 최소화 */
      .block-container {
        padding-top: 1.2rem;
      ...
      /* 메인 타이틀 가운데 정렬(Streamlit DOM에 맞춘 안전한 셀렉터) */
      .stApp h1,
      ...
      /* 제목 위/아래 여백 과하면 줄이기(스크린샷처럼 떠 보일 때) */
      div[data-testid="stMarkdownContainer"] h1 {
        margin-top: 0.2rem;
...
c1, c2, c3, c4, c5 = st.columns(5)  # KPI 5개 컬럼
...
with left:  # 좌측 컬럼 컨텍스트 시작
    ...
    # 비커버 격자만 pydeck에 올리기  # 설명 주석
    gdf_ll_unc = gdf_ll[gdf_ll["is_uncovered"] == True].copy()  # 비커버만 필터링

    # 3D 높이 + 색상(노랑→빨강) 계산  # 설명 주석
    pop = gdf_ll_unc["pop"].clip(lower=0).astype(float)  # pop 정리
    ...
    # 3D 높이(기존 로직 유지)  # 설명 주석
    gdf_ll_unc["elev"] = (np.power(pop_capped, 1.80) * 0.02).astype(float)  # 3D 높이 계산

    # 색상(노랑->빨강): t=0이면 노랑(255,255,0), t=1이면 빨강(255,0,0)  # 설명 주석
    denom = float(pop_capped.max() - pop_capped.min()) if len(pop_capped) > 0 else 0.0  # 분모
    if denom > 0:
        t = ((pop_capped - float(pop_capped.min())) / denom).clip(0, 1)  # 0~1 정규화
    else:
        t = pd.Series([0.0] * len(pop_capped), index=gdf_ll_unc.index, dtype=float)  # 전부 동일하면 0

    # RGB 계산(노랑->빨강): R=255 고정, G=255*(1-t), B=0  # 설명 주석
    gdf_ll_unc["r"] = 255
    gdf_ll_unc["g"] = (255 * (1.0 - t)).round().astype(int)
    gdf_ll_unc["b"] = 0
    gdf_ll_unc["a"] = 190  # 알파(투명도) 0~255

    grid_records = []  # pydeck 폴리곤 레코드
    for gid, popv, elev, rr, gg, bb, aa, geom in zip(  # zip으로 동시에 순회
        gdf_ll_unc[GRID_ID_COL].astype(str).tolist(),  # gid
        gdf_ll_unc["pop"].tolist(),  # pop
        gdf_ll_unc["elev"].tolist(),  # elev
        gdf_ll_unc["r"].tolist(),  # r
        gdf_ll_unc["g"].tolist(),  # g
        gdf_ll_unc["b"].tolist(),  # b
        gdf_ll_unc["a"].tolist(),  # a
        gdf_ll_unc.geometry.tolist(),  # geometry
    ):
        ...
        for poly in polys:  # 폴리곤 순회
            ...
            grid_records.append({
                "gid": gid,
                "pop": float(popv),
                "is_uncovered": True,  # 비커버만 올리므로 True로 고정
                "elev": float(elev),
                "color": [int(rr), int(gg), int(bb), int(aa)],  # 색상 필드(노랑->빨강)
                "polygon": coords
            })  # 레코드 추가
    ...
    layer_grid = pdk.Layer(  # 비커버 격자 3D 레이어
        ...
        get_elevation="elev",
        elevation_scale=1,
        get_fill_color="color",  # 레코드에 미리 계산한 color 사용
        pickable=True,
    )
...
# =========================================================
# 5-B) RIGHT: Folium  # 섹션 설명 주석
# - KPI 반경 레이어는 우측에서 제거(범례가 복잡해지는 것을 방지)  # 설명 주석
# =========================================================
...
with right:  # 우측 컬럼 컨텍스트 시작
    ...
    with st.spinner(f"{CUTOFF_MIN}분 네트워크 계산 중..."):  # 스피너
        lengths = nx.single_source_dijkstra_path_length(
            Gp, int(source_node), cutoff=float(CUTOFF_SEC), weight="travel_time"
        )  # 도달시간 계산
